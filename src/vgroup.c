...
/*   Updated: 2018/04/23 12:00:01 by gpinchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	vgroup_load(int mesh_index, int vgroup_index)
{
	...
	if (!(mesh = engine_get()->meshes[mesh_index]))
		return;
	if (!(vgroup = mesh->vgroups[vgroup_index]))
		return;
...
void	vgroup_render(int camera_index, int mesh_index, int vgroup_index)
{
	...
	vgroup = mesh->vgroups[vgroup_index];
	camera = engine_get()->cameras[camera_index];
	if (!mesh || !vgroup || !camera)
		return;
	material = engine_get()->materials[vgroup->mtl_index];
	if (!material)
		return;
	material_load_textures(vgroup->mtl_index);
	t_transform *t = engine_get()->transforms[mesh->transform_index];
	...
	glBindVertexArray(vgroup->v_arrayid);
	glDrawArrays(GL_TRIANGLES, 0, vgroup->v.size());
	glBindVertexArray(0);
	glUseProgram(0);
}
