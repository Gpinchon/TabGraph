...
void	cubemap_load_side(Texture &texture, const std::string &path, GLenum side)
{
	texture.assign(*BMP::parse(path, path), side);
}

Texture		&cubemap_load(const std::string &name, const std::string &path)
{
	Texture	&t = *Texture::create(name, new_vec2(0, 0), GL_TEXTURE_CUBE_MAP, 0, 0);
