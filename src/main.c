...
/*   Updated: 2018/04/16 18:35:03 by gpinchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	main_loop(void)
{
	...
	while (engine_get()->loop)
	{
		...
		render_scene(0);
		texture_blur(framebuffer_get_attachement(
			window_get()->render_buffer, 1), BLOOMPASS, 2.5);
		texture_blur(framebuffer_get_attachement(
			window_get()->render_buffer, 2), 1, 2.5);
		render_present(0);
