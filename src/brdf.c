...
/*   Updated: 2018/03/22 02:03:07 by gpinchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
VEC3 ImportanceSampleGGX(VEC2 Xi, VEC3 N, float roughness)
{
	...
	VEC3 up        = abs(N.z) < 0.999 ? new_vec3(0.0, 0.0, 1.0) : new_vec3(1.0, 0.0, 0.0);
	VEC3 tangent   = vec3_normalize(vec3_cross(up, N));
	VEC3 bitangent = vec3_cross(N, tangent);
	
	VEC3 sampleVec = vec3_scale(tangent, H.x);
...
VEC2 IntegrateBRDF(float NdotV, float roughness)
{
	...
	G_Vis = visibilityForLut(roughness, NdotV);
...
{
	int texture = texture_create(new_vec2(32, 32), GL_TEXTURE_2D, GL_RGB, GL_RGB);
	t_texture *t = ezarray_get_index(engine_get()->textures, texture);
	...
	while (x <= t->size.x)
	{
		...
		while (y <= t->size.y)
		{
			...
			texture_set_pixel(texture, new_vec2(1 - uv.x, uv.y), new_vec4(brdf.x, brdf.y, 0, 1));
			y++;
		}
		x++;
	}
