...
namespace PNG {
struct Struct {
	Struct(png_structp a_Ptr) : _ptr(a_Ptr) {}
...
struct Read : Struct {
	Read() : Struct(png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL)) {}
	~Read() { png_destroy_read_struct(&_ptr, nullptr, nullptr); }
};
struct Write : Struct {
	Write() : Struct(png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL)) {}
