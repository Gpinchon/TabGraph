...
void Composite()
{
	...
	float reveal = texelFetch(in_Buffer1, ivec2(gl_FragCoord.xy), 0).r;
	out_Buffer0 = vec4(accum.rgb / max(accum.a, 1e-5), reveal);
	//out_Buffer1 = texelFetch(in_Buffer2, ivec2(gl_FragCoord.xy), 0);
