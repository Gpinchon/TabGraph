...
void Framebuffer::AddColorBuffer(std::shared_ptr<Texture> attachement, unsigned mipLevel)
{
    return _impl->AddColorBuffer(attachement, mipLevel);
}

void Framebuffer::SetColorBuffer(std::shared_ptr<Texture> buffer, unsigned index, unsigned mipLevel)
{
    return _impl->SetColorBuffer(buffer, index, mipLevel);
}

void Framebuffer::SetStencilBuffer(std::shared_ptr<Texture> buffer, unsigned mipLevel)
{
    return _impl->SetStencilBuffer(buffer, mipLevel);
}

void Framebuffer::SetDepthBuffer(std::shared_ptr<Texture> buffer, unsigned mipLevel)
{
    return _impl->SetDepthBuffer(buffer, mipLevel);
}

void Framebuffer::BlitTo(std::shared_ptr<Framebuffer> to, glm::ivec2 src0, glm::ivec2 src1, glm::ivec2 dst0, glm::ivec2 dst1, BufferMask mask, TextureSampler::Filter filter)
{
...
std::shared_ptr<Texture> Framebuffer::GetColorBuffer(unsigned index)
{
